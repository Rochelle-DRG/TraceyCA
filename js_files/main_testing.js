...
$.getJSON( "ajax/columbia.json", function( data ) {
    var slides = [];
    var buttons = [];
    var mob_car_ind = [];
    var mob_car_slides = [];
    ...
    $.each(data.slides, function(k, json_slide) {
        ...
        if(json_slide.Title !== "") {
            ...
            slides.push(
                "<div class='navTitle'>" + json_slide.Title + "<br>" + json_slide.Subtitle + "</div>"
            );
            slides.push(
    ...
    var alert;
    alert("JSON failed to load, Try again later");
});
